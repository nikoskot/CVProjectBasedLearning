...
def getParser():
    parser = configargparse.ArgParser(default_config_files=["SimpleSfM\\sfmConfig.yaml"])
    parser.add("--configFile", is_config_file=True, help='config file path')
    # parser = argparse.ArgumentParser(description="Camera Calibration")
    parser.add("--dataPath", type=lambda p: Path(p).resolve(), default="SimpleSfM\\data\\images")
    # parser.add("--gtPosesPath", type=lambda p: Path(p).resolve(), default="SfM\\data\\poses.txt")
    parser.add("--resultsSavePath", type=lambda p: Path(p).resolve(), default="SimpleSfM\\sfmResults")
    parser.add("--cameraParamsFile", type=lambda p: Path(p).resolve(), default="SimpleSfM\\data\\intrinsics.json")
    return parser
...
def decompEssentialMat(E, q1, q2, K):
...
def sfm(images, K, P):
    ...
    refinedPoses = []      # Poses should be world -> camera
    points3d = []   # The 3d points that are created in the scene. List of Point3d objects
    refinedPoints3d = []   # The 3d points that are created in the scene. List of Point3d objects
    ...
    rr.log("/refinedCamera/image", rr.Pinhole(resolution=[images[0].shape[1], images[0].shape[0]], focal_length=[K[0,0], K[1,1]], principal_point=[K[0, 2], K[1, 2]]), static=True)
    ...
    for imgIdx, img in tqdm(enumerate(images)):
        ...
        rr.log("/refinedCamera/image", rr.Image(img))
    ...
    # Initialize pose of first frame
    poses.append(formTransformation(np.eye(3), np.zeros(3)))
    refinedPoses.append(formTransformation(np.eye(3), np.zeros(3)))
    projections.append(P)
    refinedProjections.append(P)
    ...
    rr.log("/refinedCamera", rr.Transform3D(mat3x3=np.linalg.inv(refinedPoses[0])[:3,:3], translation=np.linalg.inv(refinedPoses[0])[:3,3]))
    ...
    for frameIdx in range(1, len(frames)):
        print(f"---Processing frames {frameIdx-1} and {frameIdx}")
        prevFrameIdx = frameIdx-1
        currentFrameIdx = frameIdx
        
        # Estimate Essential matrix
        E, mask = cv.findEssentialMat(matches[prevFrameIdx][currentFrameIdx][0], matches[prevFrameIdx][currentFrameIdx][1], K)
        # Recover relative pose from Essential matrix
        R, t = decompEssentialMat(E, matches[prevFrameIdx][currentFrameIdx][0], matches[prevFrameIdx][currentFrameIdx][1], K)
        ...
        print(f"---Frame {currentFrameIdx}---")
        print(f"R = \n{R}")
        print(f"t = \n{t}")
        # Get the pose of the current frame
        poses.append(relativePoseTransform @ poses[prevFrameIdx])
        # Projection matrix of the current frame
        projections.append(np.concatenate((K, np.zeros((3,1))), axis = 1) @ poses[currentFrameIdx])
        
        rr.set_time("frameId", sequence=currentFrameIdx)
        # Rerun need camera -> world coordinate system transformation, so we need to invert the pose
        rr.log("/camera", rr.Transform3D(mat3x3=np.linalg.inv(poses[currentFrameIdx])[:3,:3], translation=np.linalg.inv(poses[currentFrameIdx])[:3,3]))
        rr.log("/annotatedImages", rr.Image(cv.drawKeypoints(images[currentFrameIdx], frames[currentFrameIdx].keypoints, 0, (255, 0, 0), flags=cv.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)))
        
        # Get the 3D points from the two images
        X_h = cv.triangulatePoints(projections[prevFrameIdx], projections[currentFrameIdx], matches[prevFrameIdx][currentFrameIdx][0].T, matches[prevFrameIdx][currentFrameIdx][1].T)
        X = X_h[:3, :] / X_h[3, :]
        for i in range(X.shape[1]):
            ...
            # Reprojection error
            if reprojection_error(X_point, matches[prevFrameIdx][currentFrameIdx][0][i], K, poses[prevFrameIdx]) > 2.0 or reprojection_error(X_point, matches[prevFrameIdx][currentFrameIdx][1][i], K, poses[currentFrameIdx]) > 2.0:
                continue
            
            # Create a point3d object for the 3d point
            point3dIdx = len(refinedPoints3d)
            refinedPoint3d = Point3d(X[:, i], point3dIdx, currentFrameIdx)
            # Add the id of the frames and the keypoints that the 3d point is observed at
            # refinedPoint3d.observedAt[prevFrameIdx] = matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][0][i]
            # refinedPoint3d.observedAt[currentFrameIdx] = matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][1][i]
            # # Add the id of the keypoint and the 3d point at the frames' observations
            # frames[prevFrameIdx].observations[matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][0][i]] = point3dIdx
            # frames[currentFrameIdx].observations[matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][1][i]] = point3dIdx
            
            refinedPoints3d.append(refinedPoint3d)
    
        rr.set_time("frameId", sequence=currentFrameIdx)
        rr.log("/refinedPointCloud/points", rr.Points3D([p.coords3d for p in refinedPoints3d], colors=[255, 0, 0, 255], radii=0.01))
        
        if False:
            return
        # Refinement with solvPnP
        rvec, t = cv.solvePnPRefineLM(X, matches[prevFrameIdx][currentFrameIdx][1].T, K, np.array([]), cv.Rodrigues(R)[0], t)
        R = cv.Rodrigues(rvec)
        refinedRelativePoseTransform = formTransformation(R, t.flatten())
        print(f"Refined R = \n{R}")
        print(f"Refined t = \n{t}")
        # Get the pose of the current frame
        refinedPoses.append(refinedRelativePoseTransform @ refinedPoses[prevFrameIdx])
        # Projection matrix of the current frame
        refinedProjections.append(np.concatenate((K, np.zeros((3,1))), axis = 1) @ refinedPoses[currentFrameIdx])
        
        rr.set_time("frameId", sequence=currentFrameIdx)
        # Rerun need camera -> world coordinate system transformation, so we need to invert the pose
        rr.log("/refinedCamera", rr.Transform3D(mat3x3=np.linalg.inv(refinedPoses[currentFrameIdx])[:3,:3], translation=np.linalg.inv(refinedPoses[currentFrameIdx])[:3,3]))
        
        # Get the 3D points from the two images
        X_h = cv.triangulatePoints(refinedProjections[prevFrameIdx], refinedProjections[currentFrameIdx], matches[prevFrameIdx][currentFrameIdx][0].T, matches[prevFrameIdx][currentFrameIdx][1].T)
        X = X_h[:3, :] / X_h[3, :]
        for i in range(X.shape[1]):
            X_point = X[:, i]
                    
            # In camera coordinates
            X_cam_prev = refinedPoses[prevFrameIdx] @ np.hstack([X_point, 1.0])
            X_cam_curr = refinedPoses[frameIdx] @ np.hstack([X_point, 1.0])
            # Cheirality check
            if X_cam_prev[2] <= 0 or X_cam_curr[2] <= 0:
                continue  # discard point
            ...
            def reprojection_error(X, x, K, T):
                ...
                return np.linalg.norm(x_proj[:2] - x)
            # Reprojection error
            if reprojection_error(X_point, matches[prevFrameIdx][currentFrameIdx][0][i], K, refinedPoses[prevFrameIdx]) > 2.0 or reprojection_error(X_point, matches[prevFrameIdx][currentFrameIdx][1][i], K, refinedPoses[currentFrameIdx]) > 2.0:
                continue
            
            # Create a point3d object for the 3d point
            point3dIdx = len(points3d)
            point3d = Point3d(X[:, i], point3dIdx, currentFrameIdx)
            # Add the id of the frames and the keypoints that the 3d point is observed at
            point3d.observedAt[prevFrameIdx] = matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][0][i]
            point3d.observedAt[currentFrameIdx] = matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][1][i]
            # Add the id of the keypoint and the 3d point at the frames' observations
            frames[prevFrameIdx].observations[matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][0][i]] = point3dIdx
            frames[currentFrameIdx].observations[matchesKeypointsIdxs[prevFrameIdx][currentFrameIdx][1][i]] = point3dIdx
    ...
    for i in range(X.shape[1]):
        ...
        # Add the id of the keypoint and the 3d point at the frames' observations
        frames[0].observations[matchesKeypointsIdxs[0][1][0][i]] = point3dIdx
    ...
    for frameIdx in tqdm(range(2, len(frames))):
        ...
        # Rerun need camera -> world coordinate system transformation, so we need to invert the pose
        rr.log("/camera", rr.Transform3D(mat3x3=np.linalg.inv(poses[frameIdx])[:3,:3], translation=np.linalg.inv(poses[frameIdx])[:3,3]))
        # Projection matrix of the current frame
        ...
        for prevFrameIdx in range(frameIdx-1, -1, -1):
            ...
            if len(matchesForNew3DPoints) > 0:
                ...
                for i in range(X.shape[1]):
                    ...
                    # Add the id of the keypoint and the 3d point at the frames' observations
                    frames[frameIdx].observations[matchesKeypointsIdxs[frameIdx][prevFrameIdx][0][i]] = point3dIdx
                    frames[prevFrameIdx].observations[matchesKeypointsIdxs[frameIdx][prevFrameIdx][1][i]] = point3dIdx
...
def main():
    ...
    print("---Creating path for Simple SfM results.---")
    ...
    print("---Starting Simple SfM with tracking with:")
